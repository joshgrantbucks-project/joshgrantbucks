...
#if defined(NDEBUG)
# error "Joshgrantbucks cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x14dd517c499ab76b02f4d753d71bccd030628ac43962f01ef7696178eb483d7e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Joshgrantbucks: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Joshgrantbucks Signed Message:\n";
...
{
    // Joshgrantbucks: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1159200); // Joshgrantbucks: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Joshgrantbucks: 3 days
static const int64 nTargetSpacing = 2 * 60; // Joshgrantbucks: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Joshgrantbucks: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Joshgrantbucks: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xdc8acc0ffb0d7325656c6e967c57676a3d84a9b58f1a78e00c992fb83b204631");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Dexerto 27/Feb/2021 MrBeast, Dream & more ‘ratio’ Wendy’s";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 69 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0495c29df03255e9e6ff7699158ebb129b834ec686e1de58ca79993c1aa88ba3d0435366edad8e63019452df9a20c21f93936715050d6697a98368b14649df3dff") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1614535910;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084812953;

        if (fTestNet)
        {
            block.nTime    = 1614535896;
            block.nNonce   = 386977072;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xb3dfc135d535cf5ad0fe712480b8812bf1ece9fa2ff57642a923fa15900a6090"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc1, 0xb7, 0xdd }; // Joshgrantbucks: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("JoshgrantbucksMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("JoshgrantbucksMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static JoshgrantbucksMiner(CWallet *pwallet)
{
    printf("JoshgrantbucksMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("joshgrantbucks-miner");
    ...
    try { loop {
        ...
        printf("Running JoshgrantbucksMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("JoshgrantbucksMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&JoshgrantbucksMiner, pwallet));
}
